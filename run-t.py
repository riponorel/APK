...
def run_bot(data_account, recover=1):
    ...
    try:
        ...
        gmail = f"download-mods-{random_string(6)}@gmail.com"
        slug = f"{kw}-apk-download-full-version-{random_string(6)}"
        judul = f"{kw} download apk"
        link = f"https://apps.freeplayer.one/?title= CLICK HERE >> {kw}?ref=20D"
        ...
        driver.find_element(
            By.CSS_SELECTOR, 'form > input[placeholder="Enter Link"]'
        ).send_keys(link)
        time.sleep(1)

        driver.find_element(
            By.XPATH, '//*[@id="__next"]/main/div[2]/div[1]/button'
        ).click()
